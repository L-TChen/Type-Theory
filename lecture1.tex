...
\maketitle
\end{frame}
In elementary school, we were always asked to solve
\[
  (3 + 7) * (4 + 6)= ?
\]
but the following is always correct in any sense:
\[
  (3 + 7) * (4 + 6)= (3 + 7) * (4 + 6)
\]
Why is this not the answer? It is because we are asked to \emph{reduce} an expression
to its \emph{normal form} instead of finding its meaning. We also have different
sequences of reductions:
\begin{align*}
  (3 + 7) * (4 + 6) \leadsto 10 * (4 + 6) \leadsto 10 * 10 \leadsto 100 \\
  (3 + 7) * (4 + 6) \leadsto (3 + 7) * 10 \leadsto 10 * 10 \leadsto 100. \\
\end{align*}
Two sequences above are all fine, but sometimes it makes a difference. For example, 
\[
  f(x, y) \defeq x
\]
Then,
\[
  f(0, \int _{-1}^{1}{\frac {dx}{\sqrt {1-x^{2}}}}) \leadsto \dots \leadsto ?
\]
or just
\[
  f(0, \int _{-1}^{1}{\frac {dx}{\sqrt {1-x^{2}}}}) \leadsto 0
\]
Do we really need to reduce every term we see in the expression?
We device a \emph{reduce strategy} to decide which reduction should be performed
if there many choices. 

In this lecture, we will formalise the above notions in the language of lambda
calculus.
