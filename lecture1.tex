...
\begin{definition}[$\alpha$-conversion: renaming of bound variables]
  ...
  $\to_\alpha$ is called $\alpha$-conversion. Let $=_\alpha$ be the
...
\begin{definition}[$\beta$-conversion]
  ...
  substitution is defined and $M'$ is $\alpha$-equivalent to
  $M$. 
  That is,
  \[
...
Intuitively, we say that \emph{$M$ reduces to~$N$ eventually} if there exists a
finite sequence of one-step $\beta$-reductions:
\[
  M \onereduce M_1 \onereduce \dots \onereduce N,
\]
and rename bound variables in-between. To put it
formally, $\reduce$ is a relation defined inductively by
\begin{multicols}{2}
  \begin{prooftree}
    \AXC{$M =_\alpha N$}
    \RightLabel{(0-step)}
  ...
  \begin{prooftree}
    \AXC{$M \onereduce N$}
    \AXC{$N \reduce L$}
    \RightLabel{($n+1$-steps)}
    \BIC{$M\reduce L$}
  \end{prooftree}
\end{multicols}

A reduction strategy is a procedure of selecting $\beta$-redexes
to be reduced whenever there are multiple choices. 
\begin{definition}
  A \emph{reduction strategy} is a function $F\colon\Lambda\to\Lambda$ such that
  $M \reduce F(M)$.
\end{definition}
...
\end{example}
Since the $\mathbf{Y}F$  does not really reduce to $F (\mathbf{Y}F)$ but
$\mathbf{Y}F =_\beta F(\mathbf{Y}F)$ only, here is anther fixed-point operator
such that $\Theta F \reduce F(\Theta F)$:
