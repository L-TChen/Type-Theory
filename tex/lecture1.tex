...
\subtitle{Untyped \texorpdfstring{$\lambda$}{λ}-Calculus, Part I}
\begin{document}
...
\begin{description}
  \item[abstraction]
    functions are introduced as $\lam{x} t$
...
\begin{frame}{What to expect next?}
  For $\lambda$-calculus, we will consider the following topics in programming languages in a style of mathematical formalism.
  \begin{enumerate}
    \item How are programs identified \emph{up to variable renaming}?
    ...
    \item How are programs identified \emph{computationally}?
      E.g.,
      \[
...
\begin{frame}{Representing a term as an abstract syntax tree}
  The expression $\lam{x}(\lam{y}((x@y)@z))$ is represented as
  \begin{center}
    \begin{tikzpicture}[
          level distance=1cm,
...
\begin{frame}{The need for some conventions}
  ...
  \begin{enumerate}
    ...
    \item applying a function to multiple arguments is achieved by applying a function to an argument to get another function for the next argument;
...
\begin{frame}{Binding structure in an abstract syntax tree}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The variable $x$ at the leaf is bound by its nearest enclosing abstraction over $x$ in the AST.
  Similarly for $y$.
\end{frame}
...
\begin{frame}{$\alpha$-equivalence: renaming of bound variables}
  The point of naming a variable is to determine where it applies, so
...
\begin{frame}{Pros and cons of De Bruijn representation}
  ...
  \begin{description}
    \item[Good] This representation does solve many problems:
      ...
      \end{enumerate}
    \item[Bad] Hard for humans to read.
  \end{description}
...
\begin{frame}{Third Idea: Capture-Avoiding Renaming}
  ...
  \end{align*}
  \vfill
  We say $x$ \alert{occurs} in $t$ if $x \in \Var(t)$, i.e.\ $x$ appears in $t$ somewhere.
\end{frame}
...
\begin{frame}{Renaming of bound variables}
  ...
  the fresh variable \(z\) serves as a common temporary name for the two binders.
\end{frame}
...
\begin{frame}{An example of $\alpha$-equivalent terms}
  ...
  \begin{proof}
    ...
    \begin{prooftree}
      \AXC{}
      \UIC{$(w\;y)\cdot y \mathrel{=_\alpha} (w\;x)\cdot x$}
      \UIC{$\lambda y.\, y \mathrel{=_\alpha} \lambda x.\, x$}
      \AXC{}
      \UIC{$z \mathrel{=_\alpha} z$}
      \BIC{$(\lambda y.\, y)\;z \mathrel{=_\alpha} (\lambda x.\, x)\;z$}
    \end{prooftree}
  where $(w\;y)\cdot y = w$ and $(w\;x)\cdot x = w$, so it follows that $(\lambda y.\, y)\;z \mathrel{=_\alpha} (\lambda x.\, x)\;z$.
  \end{proof}
\end{frame}
...
\begin{frame}{$\alpha$-equivalence is an equivalence}
  ...
  \end{exercise*}
  \vfill
  We are mainly interested in \alert{terms up to $\alpha$-equivalence}, as the name of a bound variable does not matter. 
...
\begin{frame}{Evaluation, informally}
  ...
  \[
    ...
    \fbox{$\cdots \underbrace{t\;\subst{u}{x}}_{\text{substitution of $u$ for $x$ in $t$}} \cdots$}
  \]
  ...
  \begin{align*}
    y \subst{t}{x} & =
      \begin{cases*}
        ...
        y & \text{otherwise.}
      \end{cases*} \\
    (u\; v) \subst{t}{x} & = (u \subst{t}{x})\;(v \subst{t}{x}) \\
    (\lambda y.\,u)\subst{t}{x} & =
                \begin{cases}
                  \lambda y.\,u & \text{if $x = y$} \\
                  \lambda y.\,(u\subst{t}{x}) & \text{if $x \neq y$ and $y \notin \FV(t)$} \\
                  \lambda z.\,(((z\,y)\cdot u)\subst{t}{x}) & \text{if $x \neq y$, $y \in \FV(t)$, and $z \notin \Var(u) \cup \FV(t) \cup \{x\}$.}
                \end{cases}
  \end{align*}

  The last clause means that we \emph{rename} the bound variable $y$ to some variable \alert{fresh} for $x$, $t$, and $u$  before proceeding.
...
\begin{frame}{Single-step $\beta$-reduction}
  ...
  A \alert{$\beta$-redex} is a term of the form $(\lam{x}t)\;u$ where computation is performed upon and the application is reduced to $t\subst{u}{x}$.

\begin{definition}
  The \emph{one-step (full) $\beta$-reduction} is a relation defined inductively by 
  \begin{multicols}{2}
    \begin{prooftree}
  ...
  \end{multicols}
\end{definition}
For example, $(\boxed{(\lam{x\,y}x)\; t})\;u \onereduce (\lam{y}t)\;u \onereduce t \qquad\text{assuming } y \notin \FV(t)$.

A term $t$ is in \alert{normal form} if $t \not \onereduce u$ for any $u$.
\end{frame}
...
\begin{frame}{Multi-step full $\beta$-reduction}
  It is convenient to represent a sequence
  of $\beta$-reductions
  \[
...
\begin{frame}{$t \reduce u$ is transitive}
  \begin{lemma}
    For every pair of derivations of $t \reduce u$ and $u \reduce v$, there
...
\begin{frame}{Summary}
  \begin{center}
    ...
    \begin{tabular}{@{}ll@{}}
      ...
      \midrule
      $x \in \Lambda(V)$
        & $x$ is a variable term over $V$ \\

      $t @ u \in \Lambda(V)$
        & application of $t$ to $u$, with $t, u\in \Lambda(V)$ \\

      $t\;u$ & conventional notation for \(t @ u\) \\

      $\lambda x.\,t \in \Lambda(V)$
        & abstraction, with $x \in V$ and $t \in \Lambda(V)$ \\

      $t =_\alpha u$
        & $t$ and $u$ have the same binding structure \\
      ...
      $t \onereduce u$
        & one full $\beta$-reduction step \\

      $t \reduce u$
        & zero or more full $\beta$-reduction steps \\

      $t =_\beta u$
        & symmetric, reflexive, transitive closure of $\onereduce$ \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{frame}
