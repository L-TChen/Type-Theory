...
\begin{frame}{Why should we study $\lambda$-calculus?}
  ...
  \begin{itemize}
    ...
    \item the methodology we develop to understand $\lambda$-calculus are used to study and design other programming languages.
  \end{itemize}
  \vfill
...
\begin{frame}{What to expect next?}
  For $\lambda$-calculus, we will consider following topics in programming languages in a style of mathematical formalism.
  \begin{enumerate}
...
\begin{frame}{Binding structure in an abstract syntax tree}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  The variable $x$ at the leaf is bound by its nearest enclosing abstraction by $x$ in the AST.
  Similarly for $y$.
\end{frame}
...
\begin{frame}{An example of $\alpha$-equivalent terms}
  ...
  \begin{proof}
    ...
    \end{prooftree}
  where $(w\;y)\cdot y$ and $(w\;x)\cdot x = w$, so it follows that $(\lambda y.\, y)\;z \mathrel{=_\alpha} (\lambda x.\, x)\;z$.
  \end{proof}
\end{frame}
