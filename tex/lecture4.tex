...
\begin{frame}{Definability, Revisited}
  ...
  \begin{proof}
    Suppose that the constant function $f(x) = \btt$ is $\lambda$-definable,
    then $R^{X \to X}(\Brackets{t}, \Brackets{t})$ by the fundamental theorem.
    

    By definition of being logical $R^{X}(\Brackets{t}(\bff),
    \Brackets{t}(\btt))$, i.e.\ $R^{X}(\btt, \btt)$---a contradiction.
