...
\begin{definition}
  ...
  \end{align*}
\end{definition}
The function extends to contexts:
$\FV(\Gamma) = \set{ X \in \mathbb{V}}{ \exists (x : A) \in \Gamma \wedge X \in \FV(A) }$.
\begin{exercise*}
...
\begin{frame}{Capture-avoiding substitution for type}
  ...
  In particular, the substitution is defined to avoid any capture of free type variables:
  \begin{definition}
  The \emph{capture-avoiding substitution} $\_\subst{A}{X}$ of a type $A$ for a type variable~$X$ is defined by 
  \begin{align*}
    X\subst{A}{X} & = A \\
